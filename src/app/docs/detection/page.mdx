...
For the Ultralytics SDK path (YOLOv8 weights, COCO128 quickstart, stable URLs for docs PRs), see [Integrations](https://github.com/bnnr-team/bnnr/blob/main/docs/integrations.md).
...
XAI is generated automatically when `xai_enabled=True` (default). For Ultralytics models, use `UltralyticsDetectionAdapter` to enable full XAI support; raw `ultralytics.nn.tasks` modules have limited XAI (see [troubleshooting.md §17](/docs/troubleshooting/)).
...
- [Troubleshooting](/docs/troubleshooting/) — Ultralytics edge cases (§17)

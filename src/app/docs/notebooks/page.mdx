...
If you want a guided student path from baseline accuracy to model understanding, start with [Cookings: Butterfly Round 1](/cookings/cooking-1/).
...
| `examples/classification/bnnr_cooking_round1_butterfly.ipynb` | Guided educational cooking: baseline, augmentations, XAI overlays, and report/event analysis | Recommended | Medium |
...
| `examples/detection/bnnr_detection_demo.ipynb` | YOLOv8 on COCO128: bbox-aware augmentations, mAP, and three-panel detection XAI (`bnnr` ≥ 0.2.6 on PyPI / Colab) | Required (live tracking) | Medium–Long |

## Recommended order
1. `bnnr_augmentations_guide.ipynb`
2. `classification/bnnr_cooking_round1_butterfly.ipynb`
3. `classification/bnnr_classification_demo.ipynb`

...
After you install BNNR (below), run `python -m bnnr demo` for a quick training showcase, then see [Model Analysis](/docs/analyze/) for `bnnr analyze` on your own checkpoint.
...
Defaults match `bnnr train` without YAML (`m_epochs=3`, `max_iterations=2`, `device=auto`, XAI on). Pass `m_epochs=1` for a fast smoke test. See [API Reference](/docs/api-reference/) and [API Reference](/docs/api-reference/).
...
**Multi-label classification:** `python -m bnnr train` with built-in datasets (`mnist`, `fashion_mnist`, `cifar10`, `stl10`, `imagefolder`, …) always uses single-label heads and `CrossEntropyLoss`. Putting `task: multilabel` in YAML **does not** switch the CLI to multi-label. Use the Python API (`task="multilabel"`, `SimpleTorchAdapter(multilabel=True)`, `BCEWithLogitsLoss`) or `examples/multilabel/multilabel_demo.py` — see [Configuration](/docs/configuration/), [Golden Path](/docs/golden-path/), [CLI Reference](/docs/cli/), and [Examples Guide](/docs/examples/).

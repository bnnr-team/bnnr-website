...
**Advanced:** optional YAML overrides — see [Configuration](/docs/configuration/). Example minimal file:
...
**Object detection:** BNNR **v0.3.0+** on PyPI supports object detection with `task="detection"`. Use `DetectionAdapter` (torchvision) or `UltralyticsDetectionAdapter` (YOLOv8) as your model adapter, with bbox-aware augmentations and mAP metrics. Detection requires the Python API — see [Detection](/docs/detection/) for the full guide and [Examples Guide](/docs/examples/) for ready-to-run scripts.

**Model analysis (new in v0.3.0):** Run `python -m bnnr analyze --model best.pt --data cifar10 --output ./out` to get a full diagnostic report (metrics, XAI, failure patterns, recommendations) on any trained model — no retraining required. See [Model Analysis](/docs/analyze/) for the complete guide.
...
Add `--config /tmp/bnnr_quickstart.yaml` when using a custom YAML file.

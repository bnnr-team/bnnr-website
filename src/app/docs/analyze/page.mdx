...
- **Detection**: Not supported by `bnnr analyze`. Use classification or multilabel only.
...
- **ROC/PR curves**: Not rendered in `report.html` yet (point metrics and diagnostics only).
- **Concept XAI**: Current analyze uses saliency/CAM-style methods; CRAFT/NMF concept-level methods are roadmap candidates.
